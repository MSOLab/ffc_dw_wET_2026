...
def test_controller_threads_valid_lb_to_neh_cp_option(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When a valid global LB is registered into ``solution_manager``, the
    controller populates ``NehCpOption.objective_lower_bound`` so the
    dispatcher can constrain the last-batch CP-SAT objective from below."""
    controller = FFcDDWSubroutineController(
        instance=_make_instance(),
        subroutine_flow=[{"method": "neh_cp", "cp_tl": 0.5, "added_batch_size": 1}],
        stopping_criteria=StoppingCriteria({"timelimit": 60.0}),
    ...
    controller.solution_manager.register(
        FFcDDWSubroutineReport(elapsed_time=0.0, obj_value=None, obj_bound=5.0),
        None,
    )
