...
## Phase-Gantt discovery `find_artifacts("mcf_lb_phase_schedule")` over-matches

In `FFcDDWReportGenerator._generate_gantt_charts`
(`src/ffc_ddw_sum_et/orchestration/reporting.py`), the MCF-LB phase-Gantt
discovery calls `self.layout.find_artifacts("mcf_lb_phase_schedule", ...)`.
That kind's `file_template` is `"{phase_name}.json"`, and
`routix.io.ArtifactLayout.find_artifacts` substitutes `*` for the free
`{phase_name}` placeholder — so the glob becomes `progress/*.json` and matches
**every** flat JSON in the instance `progress/` zone, not only MCF-LB phase
schedules. It picks up any other kind that also uses a flat `{phase_name}.json`
template in the same zone (e.g. `flip_makespan_cp_phase_schedule`, and — before
the workaround below — `csr_phase_schedule`), plus unrelated flat JSONs.

The concrete symptom (2026-06-24): the three `coarsen_solve_reconstruct` phase
Gantts were silently re-rendered by the MCF-LB loop on the MCF *shared* horizon
instead of their intended per-snapshot horizons, so
`1_coarse_solver_result` (coarsened time scale, makespan ~34) was drawn on the
original-scale axis (~1652) — visually identical to the other two.

**Workaround in place:** `csr_phase_schedule` was moved under a nested subdir
(`coarsen_solve_reconstruct/{phase_name}.json`) so the non-recursive
`progress/*.json` glob no longer reaches it (mirrors `calc_mcf_lb_phase_schedule`'s
nested layout). `flip_makespan_cp_phase_schedule` is still flat and therefore
still caught by the MCF glob — currently latent only because flip and MCF-LB
phase schedules rarely co-occur with conflicting horizons.

**Proper fix directions:**

1. Make discovery kind-specific instead of glob-by-template: have
   `find_artifacts` match only files actually registered/emitted under the
   requested kind (e.g. track emitted artifact paths per kind, or give each
   phase-schedule kind a distinguishing filename prefix/subdir so its glob is
   unambiguous), OR
2. In `_generate_gantt_charts`, filter the `mcf_lb_phase_schedule` results to
   the MCF-LB phase names the controller actually recorded
   (`controller.mcf_lb_phase_schedules`) rather than trusting the raw glob.

**Why:** The greedy glob couples unrelated artifact kinds that happen to share a
flat `{phase_name}.json` template in the same zone; any new flat phase-schedule
kind silently inherits the bug and gets mis-rendered on the MCF horizon. The
subdir workaround fixes CSR but leaves the underlying over-match in place.

**When to act:** When a third flat `{phase_name}.json` phase-schedule kind is
added in the `progress/` zone, OR when `flip_makespan_cp` phase Gantts start
co-occurring with MCF-LB phases and get drawn on the wrong horizon, OR when
`routix`'s `find_artifacts` is being tidied up and a kind-scoped match can be
bundled in. Until then, namespacing new phase-schedule kinds under their own
subdir is the cheap mitigation.
