...
bp_patch = sns.boxplot(data=diff_long, x="pair", y="diff", ax=ax1a, width=0.6, legend=False)
ax1a.axhline(y=0, color="red", linestyle="--", linewidth=1.5, label="threshold (no difference)")
...
for i, (a, b) in enumerate(bs_pairs):
    diffs = pivot[a] - pivot[b]
    ax1a.text(i, diffs.mean() + diffs.std() * 0.4,
...
sns.heatmap(pivot_hm, ax=ax1b, cmap="RdYlGn_r", annot=True, fmt=".3f",
...
cmap_rec = {str(bs): BS_COLORS[bs] for bs in batch_sizes} | {bs: BS_COLORS[bs] for bs in batch_sizes}
...
for i in range(len(hm_rec)):
    for j in range(len(hm_rec.columns)):
        ...
        ax1c.add_patch(plt.Rectangle((j - 0.5, i - 0.5), 1, 1,
                                      facecolor=color, edgecolor="white", linewidth=1.5))
        ax1c.text(j, i, f"bs={bs_val}\n{pct:.0%}", ha="center", va="center",
...
plt.tight_layout()
fig1.savefig(ANALYSIS_DIR / f"{prefix}_evidence_overview.png", dpi=200, bbox_inches="tight")
plt.close(fig1)
...
fig2.suptitle(f"Deep Dive: Batch Size 15 vs 10 Evidence ({prefix})", fontsize=16, y=0.98)

# -- 2A: diff_10vs15 distribution with threshold --
ax2a = axes[0, 0]
diff_10vs15 = pivot[10] - pivot[15]
ax2a.hist(diff_10vs15, bins=60, color="#4CAF50", alpha=0.7, edgecolor="white", linewidth=0.5, density=True)
ax2a.axvline(x=0, color="red", linestyle="--", linewidth=2, label="threshold (0 = no difference)")
ax2a.axvline(x=diff_10vs15.mean(), color="blue", linestyle="-", linewidth=1.5,
             label=f"mean={diff_10vs15.mean():.4f}")
ax2a.axvline(x=diff_10vs15.median(), color="orange", linestyle="-.", linewidth=1.5,
...
ax2a.axvspan(0, diff_10vs15.max(), alpha=0.15, color="green", label="bs15 better region")
...
"""
ax2b.text(0.05, 0.98, stats_text, transform=ax2b.transAxes,
...
for bs in batch_sizes:
    ...
    se_col = f"se_diff_{batch_sizes[0]}vs{batch_sizes[1]}" if len(batch_sizes) > 1 else None
    ax2c.plot(vals, preds, "o-", label=f"bs={bs}", color=BS_COLORS[bs],
              linewidth=2, markersize=8)
    if se_col and se_col in r_data.columns:
        se_vals = r_data[se_col].values * 1.96
        ax2c.fill_between(vals, preds - se_vals, preds + se_vals,
...
ax2e.barh(range(len(slice_plot_df)), slice_plot_df["anova_F"].values, color=colors_sig, edgecolor="white")
ax2e.set_yticks(range(len(slice_plot_df)))
ax2e.set_yticklabels(slice_plot_df["label"].values)
ax2e.axvline(x=3.0, color="orange", linestyle="--", linewidth=1.5, label="F≈3 (rough sig threshold)")
...
plt.tight_layout()
fig2.savefig(ANALYSIS_DIR / f"{prefix}_evidence_detail.png", dpi=200, bbox_inches="tight")
plt.close(fig2)
