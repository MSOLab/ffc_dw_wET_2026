...
Companion to `pw_cp_port_plan.md` and `pw_cp_port_plan_detail.md`. This
...
The plan HTMLs (`pw_cp_port_plan.html`, `pw_cp_port_plan_detail.html`)
...
### A.1. `incremental_pw_cp` composite step

- **Location**: `src/ffc_ddw_sum_et/orchestration/controller.py:2295-2417`
  (~137 lines), `metadata/20260510/incremental_pw_cp.yaml`.
- **Behaviour**: iterates `unfixed_batch_count` from
  `unfixed_batch_count_min` to `unfixed_batch_count_max`. Two policies:
  - `"always"` — single `pw_cp` pass per count.
  - `"if_no_improvement"` — repeats `pw_cp` at the current count until a
    pass leaves the incumbent's weighted E+T unchanged, then advances.
- Each inner `pw_cp` call registers its own report; the composite does
...
- **`src/ffc_ddw_sum_et/algorithm/pw_cp/option.py`** (lines 51-57,
  92-97) — `PwCpOption.horizon_makespan_multiplier: float = 1.25`,
  validated `>= 1.0`.
- **`src/ffc_ddw_sum_et/algorithm/pw_cp/dispatcher.py`** (lines 61-64)
...
**Plan coverage**: plan dispatcher pseudocode (`pw_cp_port_plan.md:135-136`)
explicitly uses
...
plan — its scope is strictly `algorithm/pw_cp/` + `controller.py` +
...
- **`pw_cp/option.py:59-71`** — two new fields on `PwCpOption`.
- **`pw_cp/dispatcher.py:187-202`** — when enabled, sets
  ...
  bloat.
- **`controller.py` `pw_cp` method** — exposes both kwargs.
- **`metadata/20260510/pw_cp_hint_check.yaml`** — dedicated scenario
...
+CONFIG_PATH = Path("metadata/20260510/pw_cp_hint_check.yaml")
```
...
via `getattr` (clarified in `pw_cp_port_plan_detail.md` §"Naming and
...
rather than the `pw_cp_grid.yaml` the plan would have implied.

## B. Code differs from plan in shape (intent preserved)

### B.1. `PwCpStepEntry` field set

Plan-listed fields (`pw_cp_port_plan.md:232-243`):
...
Code-actual fields (`pw_cp/step_log.py`):
...
- Code (`pw_cp/cp_model.py:397-457`): returns
  ...
  drift, foreseen in `pw_cp_port_plan.md:327` Risk 3). Logged at DEBUG
  per step and persisted in `PwCpStepEntry`.

### B.3. `delay_job_latest_leq_obj_contrib_all_stages` extracted helper

- Plan (`pw_cp_port_plan_detail.md:83-102`): pseudocode inlines the
...
- Code exports it as `build_operation_partition` and re-exports from
  `algorithm/pw_cp/__init__.py` so tests can call it directly.

## C. Plan items missing from code

### C.1. `docs/TODO.md` rename note for `mcf_lb_phase_schedules`

`pw_cp_port_plan_detail.md` §"Per-step Gantt" states "a TODO entry
should track renaming the container to a generic `phase_schedules`".
No such entry exists in `docs/TODO.md`.

### C.2. `pw_cp` registration in `tests/algorithm/test_algorithm_contracts.py`

Plan §Verification 2 (`pw_cp_port_plan.md:305`): "register `pw_cp` in
...
The plan's §Risks 1 (`pw_cp_port_plan.md:323`) enumerates the
...
- `delay_job_latest_leq_obj_contrib_all_stages` (two-pass right-
  justification, invariants per `pw_cp_port_plan_detail.md:104-114`).
...
- Two-phase Gantt via `_record_mcf_lb_phase("pw_cp_before" / "_after")`.
...
- `pw_cp_grid.yaml` 4-scenario grid (u∈{2,3} × pf∈{PF0,PF1}).
...
1. Add a §"Composite step: `incremental_pw_cp`" subsection to
   `pw_cp_port_plan_detail.md` describing the two policies and the
...
3. Document `log_search_progress` / `log_search_progress_max_steps` on
   `PwCpOption` and the `pw_cp_hint_check.yaml` debug scenario.
4. Correct the "dispatch table" wording in `pw_cp_port_plan.md` §"File
   layout" to match `pw_cp_port_plan_detail.md` §"Naming and placement"
   (YAML `method:` → `getattr` on controller).
5. Update `PwCpStepEntry` field list in
   `pw_cp_port_plan.md` to match `pw_cp/step_log.py` (rename
...
for revert/extraction are §A.1 (`incremental_pw_cp` to a follow-up
